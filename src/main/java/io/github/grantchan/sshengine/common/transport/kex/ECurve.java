...
public enum ECurve {
  ...
  private static final Set<ECurve> ALL = Collections.unmodifiableSet(EnumSet.allOf(ECurve.class));

  /**
   * Get a {@link ECurve} from the {@link ECParameterSpec} of a curve
   *
   * @param spec the {@link ECParameterSpec} of a curve
   * @return the matched {@link ECurve}, null if not found
   */
  public static ECurve from(ECParameterSpec spec) {
    Objects.requireNonNull(spec, "Invalid parameter - spec is null");
   ...
   */
  public static byte[] bytesOf(final ECPoint pt, final EllipticCurve curve) {
   ...
   */
  static ECPoint ecPointOf(final byte[] buf) {
